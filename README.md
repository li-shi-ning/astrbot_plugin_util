...
插件提供管理员指令组 `/麦`，用于调用麦当劳 MCP 中除下单外的接口。下单接口已在代码中硬拦截，不会作为指令暴露。

- `/麦 帮助`：查看指令说明。
- `/麦 工具`：列出当前允许调用的非下单 MCP 工具。
- `/麦 说明 <工具名>`：从 MCP 服务读取指定工具的参数 schema。
- `/麦 原始 <工具名> key=value ...`：兜底调用指定非下单工具，不需要手写 JSON。
- `/麦 时间`、`/麦 活动`、`/麦 营养`：常用无参快捷指令。
- `/麦 附近门店 城市 关键词 [到店|得来速]`：查询附近门店。
- `/麦 餐品 门店编码 [到店|麦乐送|得来速|团餐] [beCode=xxx]`：查询餐品列表。
- `/麦 估价 门店编码 商品编码[:数量],... [到店|麦乐送|得来速|团餐] [beCode=xxx]`：计算价格。
...
/麦 附近门店 上海市 人民广场 到店
/麦 餐品 123456 到店
/麦 估价 123456 10001:2,10002 到店
```

配置项 `mcd_mcp`：

- `enable_mcd_mcp_commands`：是否启用 `/麦` 指令组，默认开启。
- `url`：MCP 服务地址，默认 `https://mcp.mcd.cn/mcp-servers/mcd-mcp`。
- `token`：Bearer token 内容，直接填写 token 字符串，不需要 `Bearer` 前缀；日志只显示是否已配置，不输出 token 本体。
- `timeout_seconds`：MCP 请求超时时间。
- `max_response_chars`：单次指令回复的最大字符数，过长会截断。

接口清单和已验证的数据结构见 [docs/麦当劳MCP接口.md](docs/麦当劳MCP接口.md)。
...
- 新增 `/麦` 管理员指令组，可调用除下单外的麦当劳 MCP 接口。
