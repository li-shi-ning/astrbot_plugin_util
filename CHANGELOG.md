...
- 新增 `/麦` 管理员指令组，用于调用麦当劳 MCP 中除下单外的接口。
- 新增麦当劳 MCP 配置 `mcd_mcp`，支持开关、服务地址、直接填写 token、超时和回复长度上限。
- 新增麦当劳 MCP 中文指令与人类参数解析，常用功能不再要求手写 JSON。
- 新增麦当劳 MCP 单元测试，覆盖配置解析、下单接口拦截、人类参数解析、结果格式化和管理员权限。
