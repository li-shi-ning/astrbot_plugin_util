...
- `/群友史`：在 AIOCQHTTP 群聊中每天随机抽取一名群友，生成一段“群友史”，并以合并转发节点展示。

抽取结果按群号和当天日期稳定生成，同一天同一个群多次触发会抽到同一个对象。插件会优先读取群成员列表，并过滤机器人自身；如果平台暂时无法返回群成员列表，则退回命令发送者，避免命令无响应。

## 变更说明

- 关键词命中后从配置的本地语音目录发送语音。
- 新增 NullDox 风格的 `/土味情话 [QQ号|@用户]` 指令。
- 新增 `/群友史` 指令，用合并转发呈现每日随机群友史。
- 已移除语音生成命令。
- 已移除骰子指令处理。
