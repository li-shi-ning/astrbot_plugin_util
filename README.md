...
- `/网易云登录`、`/音乐登录`、`/点歌登录`：发送网易云音乐扫码登录二维码，扫码确认后自动保存 Cookie。
- 搜索后回复列表编号：发送歌曲信息、封面和语音播放消息。

默认 API 地址为 `http://64.90.12.120:3051`。也可以在插件配置页的 `music_search.api_base_url` 中修改；如果只填写 `host:port`，插件会自动补全为 `http://host:port`。
...
需要使用 VIP 账号访问会员音乐时，发送 `/网易云登录` 并用网易云音乐 App 扫码确认。登录成功后插件会把接口返回的 Cookie 保存到 `music_search.cookie`，后续点歌会自动携带该登录态。Cookie 属于账号凭据，请只在可信环境中使用登录指令。
...
- 新增 `/点歌` 音乐搜索与点歌功能。
- 新增 `/网易云登录` 扫码登录功能，可自动保存网易云 Cookie。
